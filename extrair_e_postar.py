...
# DICIONÁRIO DE CATEGORIAS ALINHADO AO MENU DO BLOGGER
MAPA_CATEGORIAS = {
    "ptbr": ["pt-br", "ptbr", "dublado", "legendado", "portugues", "traduzido", "pt br"],
    "Mods": ["mod", "mods", "save", "textura", "camera", "cheat", "cheto", "savedata"],
    ...
    "Espanhol": ["espanhol", "spanish", "castellano"],
...
def identificar_categorias(nome_jogo):
    ...
    if len(categorias) == 1:
        categorias.append("Luta") if "ninja" in nome_lower or "naruto" in nome_lower else categorias.append("acao")
...
def enviar_post_para_blogger(dados):
    ...
    # ESTRUTURA COMPATÍVEL COM O SEU TEMA BLOGGER
    html_content = f"""<!-- FOTO DE CAPA -->
<div class="post-cover" style="text-align: center; margin-bottom: 20px;">
    <img src="{dados['capa']}" alt="{dados['nome']}" />
</div>

BlackPostName={dados['nome']}
<!--more-->
<!-- VARIÁVEIS DE INFORMAÇÃO -->
InfoPlataforma=PPSSPP (Android / PC)
InfoIdioma=Espanhol / Inglês
InfoTamanho={dados['tamanho']}
InfoFormato={dados['formato']}
InfoMod=SaveData 100%

<!-- DESCRIÇÃO INICIAL -->
<p>Baixe agora o <b>{dados['nome']}</b> para o emulador PPSSPP no Android e PC. Jogo completo em formato {dados['formato']} ({dados['tamanho']}) com novos gráficos e jogabilidade incrível!</p>
"""
...
"""
        for i, img in enumerate(dados['prints'], start=1):
            html_content += f'        <img src="{img}" alt="{dados["nome"]} Gameplay {i}"/>\n'
...
<div class="post-dl-box">
    ...
    <a href="{link_redirecionado}" target="_blank" rel="nofollow noopener" class="btn-dl btn-dl-green">
        🚀 BAIXAR JOGO {dados['formato']} ({dados['tamanho']})
    </a>
...
<div class="tutorial-box">
    ...
    <ol class="tutorial-list">
        <li>Baixe o jogo em formato <strong>{dados['formato']}</strong> no botão acima.</li>
...
"""
    ...
    # ATRIBUI OS MARCADORES AUTOMATICAMENTE AO CAMPO "ETIQUETAS" DO BLOGGER
    msg['Category'] = ", ".join(dados['categorias'])
    ...
    try:
        ...
        server.quit()
        print(f"🎉 POST PUBLICADO: {dados['nome']} | Etiquetas: {dados['categorias']}")
    except Exception as e:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if dados['link_direto']:
                ...
                print(f"⚠️ Link direto não encontrado na URL: {url_alvo}")
    except Exception as e:
